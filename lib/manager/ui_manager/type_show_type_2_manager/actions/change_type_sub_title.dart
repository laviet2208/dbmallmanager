...
class change_type_sub_title extends StatefulWidget {
  final String subtitle;
  const change_type_sub_title({super.key, required this.subtitle});

  @override
  State<change_type_sub_title> createState() => _change_type_sub_titleState();
}

class _change_type_sub_titleState extends State<change_type_sub_title> {
  bool loading = false;
  ...
  Future<void> change_title(String title) async{
    try {
      ...
      await databaseRef.child('UI').child('productType1').child('subtitle').set(title);
      setState(() {
        loading = false;
      });
  ...
  void initState() {
    ...
    super.initState();
    titleController.text = widget.subtitle;
  }
